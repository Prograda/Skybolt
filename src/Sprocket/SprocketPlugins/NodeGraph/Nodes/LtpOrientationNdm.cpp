...
LtpOrientationNdm::LtpOrientationNdm(NodeContext* context)
{
...
std::shared_ptr<QtNodes::NodeData> LtpOrientationNdm::eval(const NodeDataVector& inputs, int outputIndex) const
{
	sim::Vector3 v(
	...
	return toNodeData<OrientationNodeData>(outputIndex, std::make_shared<sim::LtpNedOrientation>(math::quatFromEuler(v)));
}
