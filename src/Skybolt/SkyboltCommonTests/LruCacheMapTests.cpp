...
TEST_CASE("LruCacheMap put and get item")
{
	...
	int value = 5;
	CHECK(cache.putOrGet("test", value));
	CHECK(value == 5);

	int result;
	CHECK(!cache.putOrGet("test", result));
	CHECK(result == value);
}
...
TEST_CASE("LruCacheMap least recently used item is pruned when cache capacity exceeded")
{
	...
	{
		int value = i;
		CHECK(cache.putOrGet(std::to_string(i), value));
	}
	// The cache order should be [4, 3, 2, 1, 0]
	
	// Get the first item with a putOrGet()
	int value;
	CHECK(!cache.putOrGet("0", value));
	...
	CHECK(cache.putOrGet("newItem", value));
	...
	CHECK(cache.putOrGet("anotherNewItem", value));
